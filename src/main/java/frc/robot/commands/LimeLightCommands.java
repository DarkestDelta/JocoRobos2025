...
public class LimeLightCommands {
    ...
    private static final double kTaFarThreshold = 10;
    private static final double kTaCloseThreshold = 44;
    private static final double kMinStrafeSpeed = 0.01;
    private static final double kMaxStrafeSpeed = 0.04;
    private static final double kForwardSpeedFactor = 0.06;
    ...
    private double calculateForwardSpeed() {
        if (ta == 0) return 0;
        double speedMultiplier = MathUtil.clamp(1 - (ta / (kTaCloseThreshold * 1.25)), 0, 1.0);
    ...
    public void LLSeek() {
        if (hasValidTarget()) {
            if (ta < kTaFarThreshold) {
                ...
                m_robotDrive.drive(.05, strafeSpeed, 0, true);
                m_robotEndEffector.SetLLServo(0);
