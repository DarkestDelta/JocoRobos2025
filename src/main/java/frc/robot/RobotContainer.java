...
// Create an instance of ElevatorTargetCommand
ElevatorTargetCommand elevatorTargetCmd = new ElevatorTargetCommand(
    m_robotElevator,
    48,   // Stage three is 48, Stage four is 95 or 100
    0.5,  
    1.5,      
    0.1     
);
...
new Trigger(() -> m_driverController.getRawButton(1))
    .whileTrue(
        new RunCommand(() -> m_robotIntake.RaiseIntake(-1), m_robotIntake)
            .withTimeout(1.25)).whileFalse(new InstantCommand(() -> m_robotIntake.RaiseIntake(-1), m_robotIntake));

// Bind the command to Button 1
new Trigger(() -> m_driverController.getRawButton(1))
    .onTrue(
        elevatorTargetCmd
    ...
    // // .alongWith( // Also run another command alongside it
    // new RunCommand(() -> m_robotIntake.RaiseIntake(-1), m_robotIntake)
    // .withTimeout(2) // Raise intake for 2 seconds

// Use the instance method isTargetReached() from the command instance
new Trigger(() -> elevatorTargetCmd.isTargetReached())
    .onTrue(
        new RunCommand(() -> m_robotEndEffector.Shoot(0.45), m_robotEndEffector)
            .withTimeout(1.25) // Shoot for 1 second when target is reached
            .andThen(new InstantCommand(() -> m_robotEndEffector.Shoot(0), m_robotEndEffector)) // Stop shooting after timeout
    );
...
new Trigger(() -> {
  ...
  return buttonPressed;
}).onTrue( // Run while Button 2 is pressed
  new RunCommand(
      () -> {
          double axisValue = m_driverController.getRawAxis(7);
